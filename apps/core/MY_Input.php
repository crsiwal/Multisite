...
class MY_Input extends CI_Input {
    ...
    function __construct() {
        ...
        if ($this->server('REQUEST_METHOD') == 'DELETE' || $this->server('HTTP_X_HTTP_METHOD_OVERRIDE') == 'DELETE') {
            ...
            $this->_parse_request($this->delete); //parse and clean request
            ...
            $this->_parse_request($this->put); //parse and clean request
    ...
    function post_json($index = NULL, $xss_clean = FALSE) {
        if ($index === NULL AND!empty($this->post)) { // Check if a field has been provided
            $post = array();
    ...
    function delete($index = NULL, $xss_clean = FALSE) {
        if ($index === NULL AND!empty($this->delete)) { // Check if a field has been provided
            $delete = array();
    ...
    function parse_raw_http_request(array &$a_data) {
        ...
        foreach ($a_blocks as $id => $block) {
            ...
            if (strpos($block, 'application/octet-stream') !== FALSE) {
                ...
                $a_data['files'][$matches[1]] = isset($matches[2]) ? $matches[2] : "";
            } else {
